...
# Plotting Functions
def plot_raman(df, title="Raman Spectrum"):
...
def remove_background(df_sample, df_background):
    """Remove background from the Raman spectrum."""
...
def shift_counts_to_zero(df):
    """Shift the spectrum counts to start from zero."""
    df_copy = df.copy()
...
def despike_spectrum(df, moving_average, threshold=7, plot=False):
    """Remove spikes from the Raman spectrum."""
